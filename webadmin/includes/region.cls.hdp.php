...
//tblGeo.GeoID, tblGeo.strTitle AS 'Title' , tblGeo.strTags AS 'Tags', tblGeo.strLastUser AS 'Last User', tblGeo.dtLastEdit AS 'Last Edit'
      $this->ListSQL("SELECT tblRegion.RegionID, tblRegion.RegionCode AS Code, tblRegion.strRegion AS 'Region', tblGeo.strGeo AS GeoRegion, tblRegion.EN_strRegion AS 'Description EN', tblRegion.AF_strRegion AS 'Description AF', tblRegion.blnActive AS Active, tblRegion.strLastUser AS 'Last User', tblRegion.dtLastEdit AS 'Last Edit'
                     FROM tblRegion INNER JOIN tblGeo ON tblRegion.refGeoID = tblGeo.GeoID
                     WHERE 1=1 $Where
