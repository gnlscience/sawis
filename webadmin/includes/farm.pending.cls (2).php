...
//print_rr($db->Fields);
   ...
   public function getFarmTransferList($MemberID)
   {
      ...
      {//print_rr($arrFarm);
         ...
         //get all requests          
         $rst = $xdb->doQuery("SELECT * FROM tblFarm WHERE RegistrationType = '$type' AND RegistrationArgs LIKE ('%\"". $arrFarm[FarmID] ."\"%') AND strStatus NOT IN('Active') ORDER BY dtRegistered DESC, strFarm");
         while($row = $xdb->fetch())
         {//print_rr($row);
...
//echo json_encode($this->Data);exit;
   ...
   public function sendTransferAcceptNotification($FarmID)
   {
      ...
      $msg = "Test email was successfully sent to " + $TestEmailAddress;
